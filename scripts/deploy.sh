...
# 해당 파일은 code deploy 에 의해 s3 에서 ec2 로 파일 이동 이후 실행됨
# $! (special shell variable) 이용해 pid 가져오기
...
CURRENT_PID=$(cat $REPOSITORY/pid.txt)
...
nohup java -jar \
    ...
    $JAR_NAME > $REPOSITORY/nohup.out 2>&1 & echo $! > $REPOSITORY/pid.txt